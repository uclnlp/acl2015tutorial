...
object MatrixRenderer {
  ...
  case class Layout(cw: Int = 30, ch: Int = 30, rowHeaderSize: Int = 50, colHeaderSize: Int = 100, numCols: Int = -1,
                    numRows: Int = -1, fragmentStart: Int = 0){
  }

  def render(matrices: Seq[Matrix], layout: Layout = Layout()): HTML = {
    import layout._
    val numCols = if (layout.numCols == -1) matrices.flatMap(_.cells).map(_.col).max + 1 else layout.numCols
    ...
    val matrixRenderings = for ((m, mIndex) <- matrices.zipWithIndex) yield {
      ...
      val rows = for (r <- m.rowLabels) yield {
        ...
        <text x={x.toString} y={y.toString} class="row">
          {r.name}
        </text>
      }
      val cols = for (r <- m.colLabels) yield {
        ...
        <text x={x.toString} y={y.toString} class="col"
              transform={"rotate(-45 " + ((r.col + 0.5) * cw + rowHeaderSize) + "," + (colHeaderSize - ch - padding) + ")"}>
          {r.name}
        </text>
      }
      ...
      val cssClass = s"""fragment ${if (mIndex < matrices.length - 1) "current-visible" else ""}"""
      val result =
        <g class={cssClass} data-fragment-index={(mIndex + fragmentStart).toString}>
          <g>
            {cells}
          </g>
          <g>
    ...
    val html =
      <div style="text-align:center;">
        <svg class="matrix" style={s"width:${width}px; height:${height}px; display:inline; margin:auto"}>
          {matrixRenderings}
        </svg>
      </div>
